...
export const linkDoc = (id, texto, clase = 'font-mono text-sky-300') => id
    ? `<button type="button" onclick="window.abrirDetalleDocumentoGlobal(${Number(id)})" class="${clase} hover:underline cursor-pointer text-left" title="Abrir el documento">${esc(texto)}</button>`
    : esc(texto);
