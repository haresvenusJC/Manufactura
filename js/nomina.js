                    ...
                    <tbody>
                        ${data.map((n) => `
                            <tr class="border-b border-slate-900 ${n.estatus === 'cancelada' ? 'opacity-50' : ''}">
                                ...
                                <td class="p-2 font-mono">${n.poliza_id ? `<button type="button" onclick="window.verPolizaDeDocumento(${n.poliza_id}, '${n.fecha_pago || ''}')" class="text-[11px] bg-emerald-600 hover:bg-emerald-500 text-white font-semibold border border-emerald-700 px-2 py-1 rounded cursor-pointer">🧾 ${n.polizas ? n.polizas.tipo + ' #' + n.polizas.numero : 'Póliza #' + n.poliza_id}</button>` : '<span class="text-slate-500">—</span>'}</td>
